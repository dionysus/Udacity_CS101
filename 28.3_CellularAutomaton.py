...
def pattern(contribution):
	bitlist = [128, 64, 32, 16, 8, 4, 2, 1]
	patternlist = ['xxx','xx.','x.x','x..','.xx','.x.','..x','...']
	g = {}

	for i in range(0, 8):
		if contribution / bitlist[i] == 1:
			g[patternlist[i]] = 'x'
		else:
			g[patternlist[i]] = '.'
		contribution = contribution % bitlist[i]
...
def cellular_automaton(seed, num, gens):

	replacement = pattern(num)
	...
	for gen in range(0, gens):
		...
		for i in range(0, len(previousrow)):
			...
			currentrow += replacement[test]

		#print str(gen+1) + ': ' + str(currentrow)
